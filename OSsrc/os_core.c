...
#include <avr/pgmspace.h>
       INT8U  const  OSMapTbl[8]  PROGMEM  = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
#endif
/*
*********************************************************************************************************
...
#else
#include <avr/pgmspace.h>
INT8U  const  OSUnMapTbl[256] PROGMEM= {
    0, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0,       /* 0x00 to 0x0F                             */
    4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0,       /* 0x10 to 0x1F                             */
    5, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0,       /* 0x20 to 0x2F                             */
    4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0,       /* 0x30 to 0x3F                             */
    ...
    if (OSRunning == TRUE) {
        ...
        if (OSIntNesting == 0) {                           /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                #else
				y             = pgm_read_byte(OSUnMapTbl+OSRdyGrp);
...
void  OSStart (void)
{
    ...
    if (OSRunning == FALSE) {
        ...
        #else
		y             = pgm_read_byte(OSUnMapTbl+OSRdyGrp);        /* Find highest priority's task priority number   */
        x             = pgm_read_byte(OSUnMapTbl+OSRdyTbl[y]);		
		#endif
...
#else
    y                      = pgm_read_byte(OSUnMapTbl+pevent->OSEventGrp);   /* Find HPT waiting for message         */
#endif
#ifndef ALL_IN_ROM
    bity                   = OSMapTbl[y];
#else
    bity                   = pgm_read_byte(OSMapTbl+y);
#endif
#ifndef ALL_IN_ROM
    x                      = OSUnMapTbl[pevent->OSEventTbl[y]];
#else
    x                      = pgm_read_byte(OSUnMapTbl+pevent->OSEventTbl[y]);
#endif
#ifndef ALL_IN_ROM
    bitx                  = OSMapTbl[x];
#else
    bitx                   = pgm_read_byte(OSMapTbl+x);
#endif
