...
static void tx_buf_init(void)
{
	tx_buf.head 	 = 0;
	tx_buf.tail  	 = 0;
	tx_buf.disable  = 1;							//缓冲区不可用
}

static bool tx_buf_is_empty(void)					//检测发送缓冲区是否为空，1为空，0为非空
{
	return (tx_buf.tail == tx_buf.head);
}

static bool tx_buf_is_full(void)					//检测发送缓冲区是否已满，1为满，0为非满
{
...
{
	if(tx_buf_is_full())							//若发送缓冲区已满，直接返回
		return;										//是直接返回还是在这里等待？？？？？？

	TXC_DIS();										//关闭发送完成中断
	if (tx_buf.disable)								//如果发送缓冲区是不可用的
	{
		while(!(UCSRA & (1<<UDRE))); 				//如果接收数据寄存器不为空则等待
		UDR = x;									//把当前要发送的数据先发出去
		tx_buf.disable = 0;							//置发送缓冲区可用
	}
	else
	{
...
{
	if (!tx_buf_is_empty())						//判断发送缓冲区是否为空，空则置disable=1
	{
